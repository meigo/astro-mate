...
import { installAstroSkill } from './skill.js';
import { MODEL_OPTIONS } from './stack.js';
import { type VerifyResult, verify } from './verify.js';

const USAGE = `
${chalk.bold('astro-mate')} — focused Astro v6 + Tailwind v4 site generator

${chalk.bold('Usage:')}
  astro-mate new [options]                 Scaffold a site into the CURRENT directory and build it
  astro-mate fix [options]                 Iterate on an existing astro-mate project (current dir)
  astro-mate help                          Show this help
...
${chalk.bold('Workflow:')}
  ...
  astro-mate new           # then type your prompt
  # later…
  astro-mate fix           # then type your change request
`;
...
async function cmdNew(args: ParsedArgs): Promise<void> {
  ...
  if (fs.existsSync(outDir) && fs.readdirSync(outDir).length > 0) {
    ...
    log.dim('  Create and cd into an empty directory, then run astro-mate new again.');
    process.exit(1);
  }
  ...
  const skillResult = installAstroSkill(outDir);
  if (!skillResult.ok) {
...
async function cmdFix(args: ParsedArgs): Promise<void> {
  ...
  if (!fs.existsSync(path.join(dir, 'package.json'))) {
    log.fail(`No package.json in ${dir}. cd into an astro-mate project first.`);
    process.exit(1);
  }
  ...
  if (model) log.dim(`  Model: ${model}${args.model ? '' : ' (from .astro-mate.json)'}`);
  ...
  const skillResult = installAstroSkill(dir);
  if (!skillResult.ok) {
    log.warn(`Skill install failed: ${skillResult.reason}. Continuing without it.`);
  }
