...
10. **Interactivity (only if the prompt needs it):** Svelte is the designated framework. Prefer static \`.astro\`; when genuine client-side interactivity is required, install \`@astrojs/svelte\` + \`svelte\` (the versions are pinned — use the latest compatible), register \`svelte()\` in \`astro.config.mjs\`, write the component in \`.svelte\`, and hydrate with the narrowest \`client:*\` directive that works. Do NOT use React, Vue, Solid, or vanilla framework shims. New \`.svelte\` files must not break any of the four verification commands; note that neither Biome (\`.ts\`/\`.js\`/\`.json\`) nor Prettier (\`.astro\`) is configured to cover \`.svelte\`, so keep that code clean and well-formatted yourself.
11. **Accessibility (WCAG 2.1 AA):** every text/background colour pair must meet contrast — **≥4.5:1 for normal text**, **≥3:1 for large text** (≥24px, or ≥18.7px bold) and for icons/borders that convey meaning. Watch Tailwind shade pairings specifically: muted shades like \`text-*-400\`/\`text-*-500\` on light backgrounds (\`bg-white\`, \`bg-*-50\`) and \`text-*-500\`/\`text-*-600\` on dark backgrounds frequently FAIL — verify the ratio and darken/lighten a step or two if unsure. Also: use landmark elements (\`<header>\`/\`<nav>\`/\`<main>\`/\`<footer>\`), a single \`<h1>\` with logical heading order, \`alt\` text on meaningful images (empty \`alt=""\` for decorative), accessible names on links/buttons/controls, and don't remove focus outlines without a visible replacement.
